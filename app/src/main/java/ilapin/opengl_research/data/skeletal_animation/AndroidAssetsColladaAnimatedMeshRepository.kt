...
/**
 * @author ilapin on 14.03.20.
 */
class AndroidAssetsColladaAnimatedMeshRepository(private val context: Context) : AnimatedMeshRepository {
    ...
    override fun loadMesh(path: String): Mesh {
        ...
        val polylistInputsCount =
            (xPath.compile("count(/COLLADA/library_geometries[1]/geometry[1]/mesh[1]/triangles[1]/input)")
        ...
        val colladaIndices =
            (xPath.compile("/COLLADA/library_geometries[1]/geometry[1]/mesh[1]/triangles[1]/p[1]").evaluate(document) as String)
                .split(" ")
                .map { it.toInt() }
    ...
    private fun parseJointWithChildrenData(xmlDoc: Document, nodeXPath: String, jointNames: List<String>): Joint? {
        ...
        L.d(App.LOG_TAG, "Joint ${joint.name}")
        ...
        for (i in 0 until children.length) {
            ...
            L.d(App.LOG_TAG, "\tJoin child node: $childNodeName, id='$childNodeId'")
    ...
    private fun parseJointData(xmlDoc: Document, nodeXPath: String, jointNames: List<String>): Joint? {
        ...
        val jointIndex = jointNames.indexOf(jointName).takeIf { it >= 0 } ?: run {
            L.e(App.LOG_TAG, "Joint $jointName not found")
            return null
        }
    ...
    private fun parseTextureCoordinates(xmlDoc: Document): List<Vector2fc> {
        val coordinatesContainerId =
            (xPath.compile("/COLLADA/library_geometries[1]/geometry[1]/mesh[1]/triangles[1]/input[@semantic='TEXCOORD'][1]/@source").evaluate(xmlDoc) as String)
                .substring(1)
    ...
    private fun parseNormals(xmlDoc: Document): List<Vector3fc> {
        val normalsContainerId =
            (xPath.compile("/COLLADA/library_geometries[1]/geometry[1]/mesh[1]/triangles[1]/input[@semantic='NORMAL'][1]/@source").evaluate(xmlDoc) as String)
                .substring(1)
